...
namespace Data.Concrete.EfCore
{
    public class EfCoreOrderRepository: EfCoreGenericRepository<Order>,IOrderRepository
    {
        ...
        public async Task<List<TopOrdersViewModel>?> GetTop10Orders()
        {
            ...
            (
                $"SELECT TOP (10) p.Name, count(p.Id) as Sepet, sum(oi.Quantity) as SatisAdet, sum(oi.Quantity * oi.Price) as SatisTotal FROM [OrderItems] as oi inner join [Products] as p on oi.ProductId = p.Id inner join [Orders] o on o.Id = oi.OrderId inner join OrderStates os on os.Id = o.OrderStateId where os.Url != 'iptal-edildi' group by p.Name order by SatisTotal desc"
            );
        ...
        public async Task<OrderTotalViewModel?> GetOrdersTotal()
        {
            ...
            (
                $"SELECT TOP (10) sum(oi.Quantity * oi.Price) as SatisTotal FROM [OrderItems] as oi inner join [Products] as p on oi.ProductId = p.Id inner join [Orders] o on o.Id = oi.OrderId inner join OrderStates os on os.Id = o.OrderStateId where os.Url != 'iptal-edildi'"
            ).FirstOrDefaultAsync();
